...
public class ArticleDAO {
    ...
    public boolean creer(Article article) {
        String sql = "INSERT INTO article (libelle, quantite, categorie, taille, couleur, volume, poids, sl) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ...
            stmt.setBoolean(8, false); // SL = false par défaut
    ...
    public List<Article> getAll() {
        ...
        String sql = "SELECT * FROM article WHERE sl = false ORDER BY libelle";
...
private Article mapResultSetToArticle(ResultSet rs) throws SQLException {
    ...
    switch (categorie) {
        ...
        default:
            // Si catégorie inconnue, on retourne null ou on lance une exception
            System.err.println("Catégorie inconnue: " + categorie + " pour l'article " + libelle);
            return null;  // ⬅️ CORRECTION : pas de new Article()
    }
}}